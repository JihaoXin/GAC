...
| 架构类型 | SDPA head_dim | 修复有效？ | E2E 验证 |
|---------|---------------|-----------|---------|
| **直接压缩** (vanilla SVD, head_dim→d) | 不对齐 (如 d=107) | **是** (+25-28%) | 内核级 |
| **投影式** (RAP SVD, latent→head_dim) | 对齐 (如 d=128) | **否** (-0.8%) | RAP SVD E2E |
| **量化** (GPTQ, AWQ) | 不变 | N/A | N/A |

**原因**：RAP SVD 在潜在空间存储压缩表示 (d=102)，但 **SDPA 操作在投影后的 head_dim=128** 上——已经对齐。不对齐仅影响低秩投影 GEMM，不影响注意力计算本身。

**实践指南**：应用维度修复前，需验证 SDPA 是否直接操作在压缩维度上。如果压缩使用投影层恢复对齐 head_dim（如 RAP SVD），修复无 E2E 收益。

### 完成的修改任务

| 任务ID | 描述 | 状态 |
|--------|------|------|
| M1 | RAP SVD E2E 负面结果重构为架构指南 | ✓ 添加 Table 8 (Applicability) |
| M2 | ROI 数字一致性 | ✓ 统一为 3.5-5.9× |
| M3 | Figure 6 过大 | ✓ 移除，保留 Table 5 |
| m1 | Figure 3 双重强调 | ✓ 简化 caption |
| m2 | Figure 5 标签重叠 | ✓ 调整标签位置 |
| m3 | Page 6 空白 | ✓ 添加 H100 讨论 |
| m4 | 参考文献不足 | ✓ 从 12 增加到 24 |
| m5 | FlashAttention 版本说明 | ✓ 移至 Background §2.2 |

---

## 2026-01-27 更新：审稿修改第一轮

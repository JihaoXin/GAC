...
\caption{\textbf{Theoretical} dimension distribution from Fisher-information-based rank allocation (Llama-3-8B, $r$=0.8, \emph{unconstrained SVD}). 96.9\% of the 512 theoretical KV head dimensions are misaligned (only $d$=120, 3.1\%, is 8-aligned). \textbf{Note:} Production PaLU checkpoints use internal quantization that enforces alignment; our repair targets compression methods without such constraints (e.g., vanilla SVD).}
\label{fig:palu_dist}
\end{figure}
...
We define \emph{ROI} (Return on Investment) as $\text{ROI} = {\Delta\text{speedup}}/{\Delta\text{memory}}$, measuring performance gain per unit memory cost.
MINIMAL achieves 23--28\% speedup with 3.72\% overhead, yielding 6.9$\times$ ROI (i.e., ${\approx}26\%/{3.72\%}$).
$d$=120 illustrates the difference between alignment levels: it is 8-aligned ($120 \mod 8 = 0$) but not 16-aligned ($120 \mod 16 = 8$). Thus, MINIMAL (mod-8) leaves it unchanged (0\% gain), while OPTIMAL (mod-16) pads to 128, enabling Tensor Core tile optimization for +8.3\% speedup.
OPTIMAL provides additional gains at 4.0$\times$ ROI.

\subsection{Orthogonal Study: PaLU Compression Benefits}

To contextualize compression benefits (distinct from our dimension repair contribution), we compare Llama-3-8B baseline vs.\ PaLU-compressed (ratio=0.7) on A100 80GB.
Note: PaLU uses internal 32-multiple quantization, producing aligned dimensions---this evaluation shows compression benefits, not repair benefits.
Table~\ref{tab:e2e} and Figure~\ref{fig:e2e} show prefill/decode throughput.

\begin{table}[t]
\centering
\caption{LLM inference with PaLU \emph{compression} (orthogonal to dimension repair). The 11.5$\times$ decode speedup results from reduced KV cache, not alignment repair. Measurements: 200 iterations $\times$ 3 trials.}
\label{tab:e2e}
\begin{tabular}{lrrr}
\toprule
...
\caption{\textbf{Compression benefit, not repair benefit.} Prefill and decode throughput for Llama-3-8B baseline vs.\ PaLU-compressed (ratio=0.7). Note: Prefill shows minimal change (--2\%, 9870$\to$9672 tok/s), while decode exhibits dramatic 11.5$\times$ speedup (119$\to$1371 tok/s) due to KV cache compression. These results are orthogonal to our alignment repair contribution.}
\label{fig:e2e}
\end{figure}

PaLU achieves \textbf{11.5$\times$ decode throughput} by reducing KV cache size---compression benefits dominate in memory-bound decode.
The tested PaLU checkpoint uses internal alignment (32-multiple quantization), so these results are orthogonal to our repair contribution.
For compression methods without alignment constraints, kernel-level experiments (\S\ref{sec:eval}.1--3) suggest repair could provide additional 25--30\% improvement.
...
\textbf{(1) Accuracy scope}: While unit tests and theory confirm bit-exact preservation, comprehensive perplexity (WikiText-2) and downstream evaluation remain future work.
\textbf{(2) E2E integration}: Kernel-level speedups (25--30\%) are not yet integrated into PaLU's SVD structure ($W = U \cdot V^T$).
\textbf{(3) E2E impact estimate}: Our kernel-level experiments validate 25--30\% speedup on SDPA operations. The actual E2E impact depends on attention's fraction of total inference time (typically 20--40\%), suggesting 5--12\% E2E improvement for compute-bound workloads. Precise E2E validation with synthetically misaligned models is left to future work.
\textbf{(4) Validation gap}: Available PaLU checkpoints use internal alignment constraints; we validate on synthetic misaligned dimensions.
...
Theoretical analysis shows 96.9\% of unconstrained SVD dimensions would benefit; no retraining required.

End-to-end evaluation shows PaLU achieves 11.5$\times$ decode speedup from KV cache compression; notably, PaLU's internal 32-multiple quantization ensures aligned dimensions, making this benefit orthogonal to our dimension repair contribution.

...
\textbf{107} & \textbf{2.14}{\scriptsize$\pm$.06} & \textbf{2.14}{\scriptsize$\pm$.06} & \multicolumn{1}{c}{N/A$^*$} & \textbf{27.00}{\scriptsize$\pm$.20} \\
...
\begin{table}[t]
\centering
\caption{Hardware layer root cause analysis (C23 experiment). Impact measured on A100 with FP16. Values averaged over 3 trials; coefficient of variation <3\% for aligned dims, <5\% for misaligned. Absolute differences exceed measurement noise by >10$\sigma$.}
\label{tab:hardware}
\small
\begin{tabular}{@{}llrl@{}}
\toprule
...
\caption{Applicability Framework validated by contrasting experiments. Negative case (\S\ref{sec:negative_e2e}) shows --0.8\% for projection-based (as predicted). Positive case (\S\ref{sec:positive_e2e}) shows +86.9\% for direct compression (as predicted). Consult this table before applying repair.}
\label{tab:applicability}
\small
\begin{tabular}{@{}p{3.2cm}p{2.0cm}ll@{}}
\toprule
\textbf{Architecture Type} & \textbf{SDPA head\_dim} & \textbf{Repair Effect} & \textbf{Validated} \\
\midrule
\textbf{Direct compression} & \emph{Misaligned} & \textbf{Yes} & E2E \\
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & \textbf{+86.9\%} & (\S\ref{sec:positive_e2e}) \\
\midrule
\textbf{Projection-based} & \emph{Aligned} & \textbf{No} & E2E \\
(RAP SVD, latent$\to$head\_dim) & (e.g., $d$=128) & \textbf{--0.8\%} & (\S\ref{sec:negative_e2e}) \\
\midrule
\textbf{Quantization} & \emph{Unchanged} & N/A & --- \\
(GPTQ, AWQ) & (original dims) & (no change) & \\
\bottomrule
\end{tabular}
\end{table}
...
For projection-based methods (RAP SVD, some PaLU variants), misalignment affects projection GEMMs but not attention---repair provides no E2E benefit (validated: --0.8\%, Table~\ref{tab:rap_e2e}).
For direct compression (vanilla SVD, head\_dim replacement), kernel-level speedups translate to E2E gains (validated: +86.9\%, Table~\ref{tab:direct_sdpa}).
...
Our findings explain \emph{why} this alignment is necessary: without it, dimensional collapse would erode these gains.
...
\begin{table}[t]
\centering
\caption{SDPA latency (ms$\pm$std) before and after dimension repair ($B$=4, $S$=2048, $H$=32). Data from independent run vs.\ Table~\ref{tab:backend}; $\sim$6\% variance is within normal GPU measurement variability (\S\ref{sec:phenomenon}).}
...
\noindent\textbf{L1. Applicability Scope:} The 96.9\% misalignment figure is from theoretical Fisher-information analysis representing mathematically optimal ranks \emph{before} implementation constraints; all 24 production PaLU checkpoints enforce 32-multiple alignment. Our findings apply to: (i)~vanilla SVD without alignment constraints, (ii)~future compression methods relaxing alignment for accuracy gains, (iii)~unconstrained theoretical analysis informing compression design.

\noindent\textbf{L2. Downstream Task Coverage:} Zero-padding preserves model outputs exactly, as validated through perplexity measurements on RAP SVD (baseline 11.08, RAP SVD + repair 92.39, identical). However, comprehensive downstream task evaluation (MMLU, reasoning benchmarks, multi-hop QA) remains future work. While bit-exact preservation guarantees no degradation for standard dot-product attention, interaction with position encodings (RoPE, ALiBi) requires separate validation.

\noindent\textbf{L3. Hardware Generalization:} All experiments conducted on NVIDIA A100 GPUs (Ampere architecture). H100 (Hopper architecture with 4th-gen Tensor Cores, TMA, WGMMA) validation is future work. Architectural differences may alter alignment sensitivity or optimal repair strategies.

\noindent\textbf{L4. Software Version Dependency:} Results are specific to FlashAttention 2.7.4. Future versions may implement internal alignment handling, potentially reducing the performance gaps we observe. Our diagnostic framework remains relevant as guidance for compression method designers and users of current FlashAttention versions.
...
Post-training compression spans multiple paradigms: pruning (SparseGPT~\cite{sparsegpt}, LotteryTicket~\cite{lottery_ticket}), quantization (GPTQ~\cite{gptq}, AWQ~\cite{awq}, QLoRA~\cite{qlora}, LLM.int8()~\cite{llmint8}, SqueezeLLM~\cite{squeezellm}, SmoothQuant~\cite{smoothquant}, ATOM~\cite{atom}), low-rank adaptation (LoRA~\cite{lora}), SVD-based decomposition (PaLU~\cite{palu}, SVD-LLM~\cite{svdllm}, CALDERA~\cite{caldera}, SlimGPT~\cite{slimgpt}), and KV cache compression (H2O~\cite{h2o}, QUEST~\cite{quest}, PyramidKV~\cite{pyramidkv}).
...
\paragraph{Attention Optimization \& GPU Kernels.}
FlashAttention~\cite{flashattention,flashattention2,flashattention3} and its variants (xFormers~\cite{xformers}, Ring Attention~\cite{ring_attention}) optimize attention with memory-efficient algorithms but are tuned for specific dimensions $\{32, 64, 96, 128, 256\}$, with performance cliffs for other values.
MQA~\cite{mqa}, GQA~\cite{gqa}, and StreamingLLM~\cite{streaminglm} reduce KV cache while preserving standard dimensions.
GPU kernel frameworks (Triton~\cite{triton}, CUTLASS~\cite{cutlass,cutlass_paper}) enable custom optimizations but require dimension-aware programming.
The Roofline model~\cite{roofline} provides performance bounds, but irregular dimensions violate its arithmetic intensity assumptions.
...
Modern LLM serving systems (TensorRT~\cite{tensorrt}, vLLM~\cite{vllm}, TGI~\cite{tgi}, FlashInfer~\cite{flashinfer}, DeepSpeed-Inference~\cite{deepspeed,deepspeed_inference}, Orca~\cite{orca}) apply runtime optimizations but typically assume aligned dimensions.
...
GPU alignment constraints evolved with Tensor Core generations.
Volta (2017) required $K \bmod 8 = 0$ for FP16 MMA operations~\cite{volta_whitepaper}.
Ampere (2020) tightened requirements to $K \bmod 16 = 0$ for optimal m16n8k16 tiles~\cite{ampere_whitepaper}, introducing greater sensitivity to dimensional irregularities.
Hopper (2023) introduced Tensor Memory Accelerator (TMA) with cache-line-aware access patterns~\cite{nvidia_hopper_whitepaper}, potentially exacerbating alignment penalties.
...
PaLU enforces 32-multiple alignment~\cite{palu}, but this design choice is undocumented in their paper---likely discovered through empirical profiling.
...
\textbf{Our diagnostic framework retroactively explains these design decisions}: production systems converged on alignment through trial-and-error, while the root causes remained undocumented.
We provide the first systematic analysis connecting compression-induced dimensional irregularities to GPU microarchitecture constraints.

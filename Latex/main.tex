...
\begin{enumerate}
  ...
  \item \textbf{Evaluation}: Kernel-level experiments validate dimension repair achieves 25--30\% speedup with 3.7--4.7\% memory overhead on SDPA and GEMM microbenchmarks. \textbf{Note:} End-to-end integration with SVD-based compression (e.g., PaLU) is future work due to factorized weight structure challenges ($W = U \cdot V^T$) (\S\ref{sec:eval}).
\end{enumerate}
...
\caption{\textbf{End-to-end LLM inference: PaLU compression benefit.} Speedups are from compression, not dimension repair (see Scope Note above).}
\label{fig:e2e}
\end{figure}

\begin{table}[t]
\centering
\caption{\textbf{End-to-end LLM inference results.} The 11.5$\times$ decode speedup is from PaLU compression (reduced KV cache), not dimension repair (see Scope Note).}
\label{tab:e2e}
\begin{tabular}{lrrr}
\toprule
...
\textbf{(1) Accuracy validation scope:} While unit tests confirm bit-exact preservation (30/30 passed) and our theoretical analysis proves forward-pass equivalence (\S\ref{sec:eval}.5), we have not conducted comprehensive perplexity evaluation (e.g., WikiText-2) or downstream task assessment (e.g., HellaSwag, PIQA).
Zero-padding is mathematically equivalent to the original computation---the proof above guarantees $f_{\theta}(x) = \text{slice}(f_{\theta'}(x))$ for any input---so we expect no accuracy degradation.
However, large-scale validation would strengthen this claim and is left as future work.
...
\begin{enumerate}
  ...
  \item \textbf{H100 Analysis}: While our analysis focuses on A100, we expect similar alignment cliffs on H100 where TMA (Tensor Memory Accelerator) has different granularity requirements and WGMMA instructions impose their own tile constraints---potentially requiring different optimal alignment values (e.g., 32 or 64 instead of 16).
\end{enumerate}

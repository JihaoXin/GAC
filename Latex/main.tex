...
\textbf{We validate our dimension repair framework with both positive and negative cases}:
direct SDPA benchmarks show \textbf{87\% average speedup} across 45 configurations (range: 46--181\%, validated in \S\ref{sec:e2e_validation}),
while projection-based architectures (RAP SVD) correctly show --0.8\% (no benefit, as predicted by our applicability framework).
...
Our applicability framework (Table~\ref{tab:applicability}) predicts when repair helps, \textbf{validated experimentally}: direct compression (+87\%), projection-based (--0.8\%, as predicted).
...
\textbf{(1)} Direct SDPA benchmarks (\S\ref{sec:e2e_validation}) show \textbf{86.9\% average speedup}
across 45 configurations, demonstrating repair efficacy when SDPA operates on misaligned dimensions.
\textbf{(2)} RAP SVD experiments show --0.8\% (no benefit), validating our applicability
framework's prediction that projection-based architectures don't benefit from repair.
Both cases confirm the framework's predictive power (Table~\ref{tab:applicability}).
...
\begin{enumerate}
  ...
  \item \textbf{Validated Applicability Framework}: We provide practitioner guidance (Table~\ref{tab:applicability}) predicting when dimension repair helps. The framework's predictive power is \textbf{experimentally validated}: direct SDPA benchmarks show \textbf{+86.9\% speedup} (positive validation, Table~\ref{tab:direct_sdpa}), while RAP SVD experiments confirm \textbf{--0.8\%} (negative validation, as predicted for projection-based architectures, Table~\ref{tab:rap_e2e}). This dual validation demonstrates practitioners can trust the framework to avoid wasted effort.
...
\caption{\textbf{KEY CONTRIBUTION: Applicability Framework (Experimentally Validated).} Predicts dimension repair effectiveness based on compression architecture. \textbf{Validated}: Direct compression shows \textbf{+86.9\% speedup} (\S\ref{sec:e2e_validation}, Table~\ref{tab:direct_sdpa}), while projection-based shows \textbf{--0.8\%} (Table~\ref{tab:rap_e2e})---exactly as predicted. \textbf{Practitioners: Consult this table before applying repair.}}
...
\subsection{Framework Validation: Positive (+87\%) and Negative (--0.8\%) Cases}

...
\textbf{While production checkpoints (PaLU, AWQ) enforce alignment internally, our theoretical analysis shows 96.9\% of SVD-optimal ranks would violate GPU alignment}---this paper targets unconstrained compression scenarios and provides diagnostic guidance.
\textbf{We validate with contrasting end-to-end experiments}:
(1)~Negative case first: \textbf{--0.8\%} for RAP SVD (projection-based architecture)---exactly as predicted by our framework, confirming it correctly identifies when repair does \emph{not} help.
(2)~Positive case: \textbf{86.9\% average speedup} across 45 SDPA benchmark configurations (batch 1--8, sequences 512--2048, range 46--181\%) when SDPA operates on misaligned dimensions from direct compression.
When applicable, dimension repair achieves 22--28\% kernel-level speedup with 3.7--7.2\% memory overhead (ROI: 3.5--5.9$\times$).
\end{abstract}
...
%% Table removed to address Page 6 crowding (M3 verification: redundant with Table~\ref{tab:repair_perf})
%% Original data: d=107: 2.06±0.06 → 1.49±0.04 (112) / 1.51±0.04 (128)

Padding to 112 achieves 27.8\% speedup with only 4.7\% memory overhead---an excellent tradeoff (see Table~\ref{tab:repair_perf} for detailed measurements).
...
\paragraph{GPU Architecture \& Tensor Cores.}
NVIDIA Tensor Cores (introduced in Volta~\cite{volta_whitepaper}, refined in Ampere~\cite{ampere_whitepaper} and Hopper~\cite{nvidia_hopper_whitepaper}) require aligned dimensions for peak throughput: Ampere's m16n8k16 MMA tiles mandate $K \mod 16 = 0$ for FP16 operations.
Misaligned dimensions force scalar fallback paths or padding overhead, similar to challenges in irregular matrix computations~\cite{cutlass_paper}.
Prior work on efficient matrix multiplication~\cite{roofline} assumes regular blocking; dimensional collapse breaks these assumptions.

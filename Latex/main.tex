...
Large Language Models (LLMs) deliver strong capabilities, but their scale hinders deployment~\cite{llama3}.
Post-training compression reduces model size and can be categorized into three families: \emph{quantization}~\cite{gptq,awq}, \emph{sparsification}~\cite{sparsegpt,wanda}, and \emph{dimension reduction}~\cite{asvd,palu,llmpruner,h2o,pyramidkv}.
...
To see why, consider the one of the core operators in LLMs, GEMM (General Matrix Multiply) $Y{=}X{\cdot}W$ (Figure~\ref{fig:gemm_dims}), with sequence dimension~$M$, inner dimension~$K$, and output dimension~$N$.
Three compression methods each target a different axis:
\textbf{(i)}~low-rank factorization replaces $W$ with two factors $A{\cdot}B$, introducing a reduced rank $r$ as the inner dimension of two smaller GEMMs~\cite{asvd,palu,svdllm2024};
\textbf{(ii)}~structured pruning removes neurons or attention heads, shrinking~$N$~\cite{llmpruner,sparsegpt,wanda};
\textbf{(iii)}~token/KV eviction drops sequence entries, reducing~$M$~\cite{h2o,pyramidkv}.
...
Prior hardware-aware methods~\cite{halp2021,haloc2023} treat latency as a black-box signal, tying solutions to specific CNN architectures without isolating root causes or providing parameter-budget guarantees.
...
GACis a compressor-agnostic framework that restores speed without changing the upstream compressor's design.
...
In Figure~\ref{fig:sdpa_latency}, alternating shades mark template regions; labels such as ``$t{=}96$, $128{\times}64$'' show the active template and tile shape.
\begin{figure}[t]
  \centering
...
$\mathcal{F}$ can be any established dimension-reducing compressor---e.g., ASVD~\cite{asvd} (SVD factorization) or LLM-Pruner~\cite{llmpruner} (structured pruning).
...
Dimension-reducing compressors such as SVD factorization~\cite{asvd,svdllm2024,palu}, structured pruning~\cite{sparsegpt,wanda,llmpruner}, and KV eviction~\cite{h2o,quest,pyramidkv} optimize accuracy under a size budget but ignore how the resulting dimensions interact with GPU execution.
HALP~\cite{halp2021} and HALOC~\cite{haloc2023} incorporate hardware awareness into CNN compression, but treat latency as a \emph{black-box} signal: they optimize aggregate runtime without isolating \emph{why} certain dimensions are slow, and are tied to specific CNN architectures with no parameter-budget guarantee.
...
(${\sim}$30\% overhead; \S\ref{sec:framework}); vLLM~\cite{vllm} rejects unsupported head dimensions.
...
Extending to more variaties(larger scales, MoE architectures, etc.) would test GAC's generality.

\noindent \textbf{Hardware diversity.}
All experiments use Ampere (A100).
Newer GPUs impose stricter alignment~\cite{nvidia_tensor_core_evolution2024,nvidia_hopper_whitepaper,flashattention3};
...
Validating GAC under optimized serving engines (e.g., vLLM~\cite{vllm}, TensorRT~\cite{tensorrt}) and DL compilers (e.g., TVM) is needed to confirm alignment gains carry over to production stacks.

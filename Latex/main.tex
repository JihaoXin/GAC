...
We propose a \emph{dimension repair} pass achieving 22--28\% kernel-level speedup with 3.7--7.2\% memory overhead (return on investment: speedup\%/overhead\% = 3.5--5.9$\times$).
Crucially, repair efficacy is \emph{architecture-dependent}: it helps when SDPA operates directly on compressed dimensions, but not for projection-based architectures---we provide an applicability framework (Table~\ref{tab:applicability}) guiding practitioners on when repair applies.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{\textbf{Applicability Framework}: When does dimension repair help? The key factor is whether SDPA operates on compressed or projected dimensions. \textbf{Use this table to determine if repair applies to your compression method.}}
\label{tab:applicability}
\small
\begin{tabular}{@{}llcc@{}}
\toprule
Architecture Type & SDPA head\_dim & Repair Helps? & Validated \\
\midrule
\textbf{Direct compression} & \emph{Misaligned} & \colorbox{green!20}{\textbf{Yes}} (+25--28\%) & Kernel (\S\ref{sec:repair_validation}) \\
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & & \\
\midrule
\textbf{Projection-based} & \emph{Aligned} & \colorbox{red!15}{\textbf{No}} (--0.8\%) & E2E (\S\ref{sec:e2e_validation}) \\
(RAP SVD, latent$\to$head\_dim) & (e.g., $d$=128) & & \\
\midrule
\textbf{Quantization} & \emph{Unchanged} & \colorbox{gray!15}{N/A} & N/A \\
(GPTQ, AWQ) & & & \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{E2E Architectural Validation: When Repair Does Not Help}
\label{sec:e2e_validation}

\textbf{As predicted by our applicability framework (Table~\ref{tab:applicability})}, dimension repair provides no benefit for projection-based architectures.
We validate this architectural insight through E2E inference benchmarks on RAP SVD~\cite{rap} compressed Llama-3-8B ($d$=102, 100\% misaligned).
Table~\ref{tab:rap_e2e} confirms the framework's prediction.
...
\textbf{Note}: Results are specific to FlashAttention 2.7.4 on A100; future versions may implement internal alignment handling.
...
\textbf{Note}: All results are specific to FlashAttention 2.7.4; future versions may implement internal alignment handling, potentially reducing the performance gaps we observe.

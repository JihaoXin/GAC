...
\begin{enumerate}
  ...
  \item \textbf{Evaluation}: Kernel-level experiments on SDPA and GEMM validate that dimension repair recovers 25--30\% performance with only 3.7--4.7\% memory overhead. End-to-end integration with PaLU's SVD structure is identified as future work (\S\ref{sec:eval}).
\end{enumerate}
...
MINIMAL & mod 8 & 3.72\% \\
OPTIMAL & mod 16 & 7.20\% \\
PREDEFINED$^\dagger$ & \{64,96,112,128\} & 7.20\% \\
\bottomrule
\multicolumn{3}{l}{\footnotesize $^\dagger$PREDEFINED maps to same targets as OPTIMAL for PaLU dimensions}\\
\multicolumn{3}{l}{\footnotesize (114--125), thus shares identical benchmark results.}
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{SDPA latency (ms) before and after dimension repair ($B$=4, $S$=2048, $H$=32). Note: D=107 baseline (2.064ms) differs slightly from Table~\ref{tab:padding} (2.192ms) due to different experiment runs; we report both for completeness.}
...
\textbf{Important clarification:} The speedups reported here are from \emph{PaLU compression} (reduced KV cache), not from dimension repair.
Applying our repair pass to PaLU's SVD decomposition ($W_{kv} = U \cdot V^T$) requires adapting to the factorized structure where per-head dimensions reside in $U$ matrices, which we leave to future work.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig6_e2e.pdf}
\caption{\textbf{End-to-end LLM inference: PaLU compression benefit.} These speedups are from compression (reduced KV cache), \emph{not} dimension repair. See Table~\ref{tab:e2e} for details and methodology notes.}
\label{fig:e2e}
\end{figure}

\begin{table}[t]
\centering
\caption{\textbf{End-to-end LLM inference: PaLU compression benefit (NOT from dimension repair).} The 11.5$\times$ decode speedup results from reduced KV cache size, not from dimension alignment. Our dimension repair pass was not applied because PaLU's SVD factorization requires specialized adaptation (\S\ref{sec:eval}.4).}
\label{tab:e2e}
\begin{tabular}{lrrr}
\toprule
...
\textbf{(1) Accuracy validation scope:} While unit tests confirm bit-exact preservation (30/30 passed), we have not conducted comprehensive perplexity evaluation (e.g., WikiText-2) or downstream task assessment (e.g., HellaSwag, PIQA).
Zero-padding is mathematically equivalent to the original computation, so we expect no accuracy degradation---but large-scale validation would strengthen this claim.
...
Future work includes integrating alignment constraints directly into compression optimization, extending the repair pass to SVD decomposition structures, and analysis on H100.
While our analysis focuses on A100, we expect similar alignment cliffs on H100 where TMA (Tensor Memory Accelerator) has different granularity requirements and WGMMA instructions impose their own tile constraints---potentially with different optimal alignment values.

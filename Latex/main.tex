...
We propose a \emph{dimension repair} pass achieving 25--28\% kernel-level speedup with 3.7\% memory overhead (5.9--6.7$\times$ ROI).
...
\begin{enumerate}
  ...
  \item \textbf{Evaluation}: Kernel-level experiments demonstrate 25--28\% speedup with 3.7\% memory overhead---a 5.9--6.7$\times$ return on investment (speedup per unit memory cost) (\S\ref{sec:eval}).
\end{enumerate}
...
\caption{Dimension distribution from \emph{unconstrained} Fisher-information-based rank allocation (Llama-3-8B, $r$=0.8). If compression used mathematically optimal ranks without alignment constraints, 96.9\% of 512 KV head dimensions would be misaligned. \textbf{Note:} All production PaLU checkpoints enforce 32-multiple alignment (\S\ref{sec:scope}); this figure motivates studying methods that relax such constraints. (See ``THEORETICAL ANALYSIS'' banner in figure.)}
\label{fig:palu_dist}
\end{figure}
...
\textbf{Context (not dimension repair)}: We compare Llama-3-8B baseline vs.\ PaLU (ratio=0.7) to contextualize compression benefits (Table~\ref{tab:e2e}, Figure~\ref{fig:e2e}).
...
For methods \emph{without} alignment constraints (e.g., vanilla SVD), kernel-level experiments (C4) suggest additional 25--30\% improvement from repair.
...
\caption{\textbf{PaLU compression benefit (orthogonal to dimension repair)}. The 11.5$\times$ decode speedup is from KV cache compression, \emph{not} dimension alignment. PaLU enforces 32-multiple alignment; this figure contextualizes compression benefits, not repair.}
\label{fig:e2e}
\end{figure}
...
\subsection{Limitations and E2E Validation}
\label{sec:limitations}

\textbf{(1) Architecture-dependent repair efficacy}: We ran E2E inference benchmarks on RAP SVD compressed Llama-3-8B ($d$=102, 100\% misaligned).
Surprisingly, dimension repair showed no benefit (prefill: --1.5\%, decode: --0.7\%).
Root cause analysis revealed that RAP SVD stores compressed representations in latent space ($d$=102), but \emph{SDPA operates on projected head\_dim=128}---which is already aligned.
The misalignment only affects the low-rank projection GEMMs ($W_A$: hidden$\to$latent, $W_B$: latent$\to$head\_dim), not the attention computation itself.
This explains why kernel-level SDPA benchmarks (C4) show 25--30\% speedup while E2E RAP SVD shows none: the bottleneck differs by architecture.

\textbf{Implication}: Dimension repair is effective when SDPA directly operates on misaligned dimensions (e.g., direct head\_dim compression without projection), but \emph{not} for architectures that project back to aligned dimensions before attention.
Our kernel-level microbenchmarks (C4) remain valid for the former scenario; practitioners should verify their compression architecture.

\textbf{(2) Scope}: The 96.9\% figure is from theoretical Fisher-information analysis; all 24 production PaLU checkpoints enforce 32-multiple alignment.
Our findings apply to methods without alignment constraints (vanilla SVD, RAP SVD) or future methods that relax constraints for better compression.
\emph{Note: Results are specific to FlashAttention 2.7.4; future versions may implement internal alignment handling.}

\textbf{(3) Downstream}: Zero-padding preserves outputs exactly; perplexity validated on RAP SVD. Comprehensive task evaluation (MMLU, etc.) is future work.
...
\emph{Quantitative comparison with TensorRT's runtime padding is future work}---our hypothesis is that compile-time repair eliminates repeated padding overhead, but this requires controlled benchmarking against TensorRT's internal implementation.
vLLM and TGI rely on FlashAttention/xformers; our slow-path findings apply directly.
...
Our dimension repair achieves 25--28\% kernel-level speedup with 3.7\% memory overhead when SDPA operates directly on misaligned dimensions.

\textbf{Limitation}: RAP SVD E2E validation showed no benefit because its architecture projects to aligned head\_dim before SDPA.
Practitioners should verify their compression architecture before applying repair.
PaLU's internal 32-multiple alignment makes its 11.5$\times$ decode speedup orthogonal to repair.
H100+ generalization remains future work.

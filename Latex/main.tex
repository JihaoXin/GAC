...
We propose a \emph{dimension repair} pass achieving 22--28\% kernel-level speedup with 3.7--7.2\% memory overhead (return on investment: speedup\%/overhead\% = 3.5--5.9$\times$ for architecture-dependent SDPA scenarios).
...
\midrule
96  & 1.17{\scriptsize$\pm$.03} & 1.12{\scriptsize$\pm$.02} & 2.38{\scriptsize$\pm$.05} & 26.0{\scriptsize$\pm$.20} \\
104 & 1.54{\scriptsize$\pm$.04} & 1.54{\scriptsize$\pm$.04} & 2.75{\scriptsize$\pm$.06} & 26.5{\scriptsize$\pm$.20} \\
\textbf{107} & \textbf{2.14}{\scriptsize$\pm$.06} & \textbf{2.14}{\scriptsize$\pm$.06} & \multicolumn{1}{c}{N/A$^*$} & \textbf{27.0}{\scriptsize$\pm$.20} \\
112 & 1.53{\scriptsize$\pm$.04} & 1.53{\scriptsize$\pm$.04} & 2.60{\scriptsize$\pm$.05} & 27.1{\scriptsize$\pm$.20} \\
128 & 1.47{\scriptsize$\pm$.03} & 1.47{\scriptsize$\pm$.03} & 2.55{\scriptsize$\pm$.05} & 28.1{\scriptsize$\pm$.20} \\
\bottomrule
...
\midrule
\textbf{Direct compression} & \emph{Misaligned} & \textbf{Yes} (+25--28\%) & Kernel (\S\ref{sec:repair_validation}) \\
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & & \\
\midrule
\textbf{Projection-based} & \emph{Aligned} & \textbf{No} (--0.8\%) & E2E (\S\ref{sec:e2e_validation}) \\
...
\subsection{E2E Architectural Validation}
\label{sec:e2e_validation}

We validate the applicability framework (Table~\ref{tab:applicability}) through E2E inference benchmarks on RAP SVD~\cite{rap} compressed Llama-3-8B ($d$=102, 100\% misaligned).
Table~\ref{tab:rap_e2e} shows the results.
...
This validates our architectural applicability framework: kernel-level SDPA benchmarks show 25--30\% speedup for misaligned dimensions (Table~\ref{tab:repair_perf}), but E2E benefit requires SDPA to operate directly on those dimensions.
...
RAP SVD & Any integer & \textbf{Susceptible} \\
\midrule

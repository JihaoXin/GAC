...
The key insight is that model inference becomes slower because the resulting dimensions are incompatible with the GPU execution stack.
...
\label{sec:analysis}
We analyze where and why dimensional misalignment causes slowdowns on an NVIDIA A100-80GB with PyTorch 2.9.1, CUDA 12.8, FP16 (results for H100-80GB are provided in Appendix~\ref{app:h100}).
...
We evaluate on Llama-3-8B~\citep{llama3} with $\rho=15\%$ using an NVIDIA A100-80GB with PyTorch 2.9.1, CUDA 12.8, FP16 (see Appendix~\ref{app:h100} for H100 results).
...
We report perplexity (WikiText-2) and two downstream tasks (PiQA, HSwag; 200 samples each).
...
While our main experiments use Ampere (A100), we also provide results on Hopper (H100) in Appendix~\ref{app:h100}, which shows even stricter alignment penalties.
...
\label{app:h100}
To demonstrate that dimensional misalignment is not specific to the Ampere architecture, we replicate our key profiling and evaluation experiments on an NVIDIA H100-80GB GPU. The Hopper architecture introduces new Tensor Core instructions (e.g., TMA) and different cache hierarchies, which impose even stricter alignment constraints.

Figure~\ref{fig:sdpa_latency_h100} shows the SDPA latency on H100. Similar to A100, we observe a staircase pattern, but the penalties for misalignment are even more pronounced due to the higher peak performance of the H100.

Figure~\ref{fig:gemm_alignment_h100} illustrates the GEMM latency with dimension sweeps on H100. The kernel-switching cliffs and alignment penalties remain significant.

Figure~\ref{fig:hw_alignment_h100} details the hardware-level alignment penalties on H100, showing the impact on Tensor Core throughput and L2 Cache bandwidth.

Finally, Figure~\ref{fig:prefill_scaling_h100} shows the end-to-end prefill latency scaling on H100. The unaligned models suffer from severe slowdowns, while GAC successfully restores the expected speedup by enforcing hardware-aligned dimensions.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures_h100/fig2_sdpa_latency_h100.pdf}
...
\begin{figure*}[h]
\centering
\includegraphics[width=\textwidth]{figures_h100/fig_gemm_alignment_h100.pdf}
\caption{GEMM latency with dimension sweep on H100.}
\label{fig:gemm_alignment_h100}
\end{figure*}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures_h100/fig_prefill_scaling_h100.pdf}

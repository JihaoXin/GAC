...
We systematically diagnose three root causes: Tensor Core tile misalignment (58\% slowdown), vectorized load degradation (50\% loss), and SDPA bandwidth inefficiency (40\%).
We propose a \emph{dimension repair} pass achieving 22--28\% kernel-level speedup with 3.7--7.2\% memory overhead (ROI: 3.5--5.9$\times$).
\textbf{Scope}: In \emph{unconstrained} compression scenarios (e.g., vanilla SVD, future methods without quantization constraints), our theoretical analysis shows 96.9\% of dimensions would be misaligned; production PaLU checkpoints already enforce 32-multiple alignment internally.
Repair efficacy is \emph{architecture-dependent}: it helps when SDPA operates directly on compressed dimensions, but not for projection-based architectures---we provide an applicability framework (Table~\ref{tab:applicability}) that correctly predicts when repair applies. Our RAP SVD experiments validate this framework: the predicted --0.8\% (no benefit) for projection-based architectures was confirmed experimentally, demonstrating the framework's predictive accuracy.
\end{abstract}
...
\subsection{Framework Validation: Successful Prediction}
\label{sec:e2e_validation}

A key contribution of our applicability framework (Table~\ref{tab:applicability}) is its ability to predict repair efficacy \emph{before} implementation---enabling practitioners to avoid wasted engineering effort on incompatible architectures.

\textbf{Hypothesis}: For projection-based compression methods like RAP SVD~\cite{rap}, SDPA operates on the \emph{projected} head\_dim (128, already aligned), not the compressed latent space ($d$=102). Our framework predicts \textbf{no benefit from dimension repair}.

\textbf{Experiment}: We tested RAP SVD compressed Llama-3-8B ($d$=102, 100\% misaligned latent dimensions) with and without dimension repair.

\textbf{Result}: Prefill shows --0.8\%, Decode --0.9\% (Table~\ref{tab:rap_e2e})---\emph{exactly as predicted}. The repair provides no benefit because SDPA never sees the misaligned dimensions.

\textbf{Significance}: This negative result \emph{validates} our framework. Practitioners can consult Table~\ref{tab:applicability} to determine applicability \emph{before} investing engineering effort. The framework correctly distinguished between direct compression (repair helps, +25--28\%) and projection-based architectures (repair unnecessary).
...
\textbf{Architectural Insight: Why Projection-Based Methods Don't Benefit.}
...
\textbf{This validates our applicability framework}: the ``Projection-based'' row of Table~\ref{tab:applicability} correctly predicts no E2E benefit.
Kernel-level SDPA benchmarks show 25--30\% speedup for misaligned dimensions (Table~\ref{tab:repair_perf}), but E2E benefit requires SDPA to operate directly on those dimensions---a key architectural distinction.

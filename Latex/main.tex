...
\begin{abstract}
Post-training compression techniques for Large Language Models (LLMs) can produce irregular tensor dimensions (e.g., \texttt{head\_dim=107} instead of 128) when unconstrained.
...
In this \emph{theoretical} scenario, 96.9\% of the compressed dimensions are not 8-aligned.\footnote{This analysis uses theoretical optimal ranks from Fisher information~\cite{palu} before any implementation-specific constraints. We verified that all 24 publicly available PaLU checkpoints (ratio 0.5--0.9) use \emph{aligned} dimensions due to internal quantization. Our dimension repair addresses vanilla SVD compression without such constraints.}
...
\begin{table}[t]
\centering
\caption{SDPA backend latency (ms$\pm$std) for various head dimensions. ``---'' indicates MEM\_EFFICIENT backend unavailable due to strict 8-alignment requirement ($d$=107 is not 8-aligned). Measurements: 200 iterations $\times$ 3 trials.}
\label{tab:backend}
\begin{tabular}{lrrrr}
\toprule
...
\textbf{107} & \textbf{2.14}{\scriptsize$\pm$.06} & \textbf{2.14}{\scriptsize$\pm$.06} & \multicolumn{1}{c}{---} & \textbf{27.0}{\scriptsize$\pm$.2} \\
...
\subsection{End-to-End Compression Baseline (Not Repair)}

\noindent\fbox{\parbox{0.95\columnwidth}{\textbf{Scope Note:} This section shows PaLU \emph{compression} benefits (reduced KV cache), \textbf{not} dimension repair effects. Available PaLU checkpoints already use aligned dimensions due to internal quantization. Repair integration remains future work.}}
...
(2) \emph{E2E integration}: Kernel-level speedups (25--30\%) are not yet integrated into PaLU's SVD structure ($W = U \cdot V^T$).
(3) \emph{E2E impact estimate}: Our kernel-level experiments validate 25--30\% speedup on SDPA operations. Since attention constitutes approximately 30\% of LLM inference time~\cite{flashattention2}, we estimate E2E improvements of \textbf{7--9\% for prefill} and \textbf{2--3\% for decode} (where memory bandwidth dominates). This extrapolation assumes similar alignment benefits across layers.
(4) \emph{Validation gap}: Available PaLU checkpoints use internal alignment constraints; we validate on synthetic misaligned dimensions.
